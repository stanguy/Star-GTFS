...
You want to initialize the database with the official dataset: Fetch the zip file from the Keolis website and unzip it in @tmp/@, set an environment variable @KEOLIS_API_KEY@ with, well, an API key from Keolis, and just run @rails runner script/import.rb@.
...
PostgreSQL is the only fully supported database, due to the growing (helpfully) use of PostGIS features (and so, it also requires a configured postgis server and database)
...
The Vincenty bearing computation method is derived from a work by Chris Veness, licensed under CC-BY.