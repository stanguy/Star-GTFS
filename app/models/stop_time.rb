    ...
    if now.day != later.day
      where( :line_id => line_id ).
        where( "( calendar_id IN (?) AND arrival > ? ) OR ( calendar_id IN (?) AND arrival < ? )",
              Calendar.from_time( now ).select(:id), value_now,
              Calendar.from_time( later ).select(:id), value_later )
    elsif now.hour < 8
      where( :line_id => line_id ).
        where( "( calendar_id IN (?) AND arrival > ? ) OR ( calendar_id IN (?) AND arrival > ? AND arrival < ? )",
              Calendar.from_time( now - 1.day ).select(:id), value_now + 24.hours,
              Calendar.from_time( now ).select(:id), value_now, value_later )
    else
      where( :line_id => line_id ).
        where( calendar_id: Calendar.from_time( now ).select(:id) ).
